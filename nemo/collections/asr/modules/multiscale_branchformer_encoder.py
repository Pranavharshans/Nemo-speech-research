...
from nemo.collections.asr.parts.submodules.branchformer_modules import MultiScaleBranchformerLayer

__all__ = ['MultiScaleBranchformerEncoder']


class MultiScaleBranchformerEncoder(BranchformerEncoder):
    """
    Branchformer encoder with a multi-scale cgMLP local branch.

    This variant is intended for offline training/inference.
    """
    ...
    def __init__(
        ...
        if cgmlp_scale_kernels is None:
            center_kernel = self.cgmlp_conv_kernel
            cgmlp_scale_kernels = [9, center_kernel, 63]
        self.cgmlp_scale_kernels = list(cgmlp_scale_kernels)
        if any(kernel_size <= 0 or kernel_size % 2 == 0 for kernel_size in self.cgmlp_scale_kernels):
            raise ValueError(f"cgmlp_scale_kernels must be positive odd integers, got {self.cgmlp_scale_kernels}")
        ...
        for layer_idx in range(n_layers):
            self.layers[layer_idx] = MultiScaleBranchformerLayer(
                d_model=d_model,
                d_ff=d_ff,
                cgmlp_linear_units=cgmlp_linear_units,
                cgmlp_scale_kernels=self.cgmlp_scale_kernels,
                self_attention_model=self_attention_model,
                global_tokens=global_tokens,
                global_tokens_spacing=global_tokens_spacing,
                global_attn_separate=global_attn_separate,
    ...
    def get_layer_type(self, layer_idx: int) -> str:
        if isinstance(self.layers[layer_idx], MultiScaleBranchformerLayer):
            return "multiscale_branchformer"
        return "branchformer"
