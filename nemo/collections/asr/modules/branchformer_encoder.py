...
from nemo.collections.asr.parts.submodules.branchformer_modules import BranchformerLayer

__all__ = ['BranchformerEncoder']


class BranchformerEncoder(ConformerEncoder):
    """
    Branchformer / E-Branchformer style encoder that keeps the existing NeMo
    FastConformer front-end and positional encoding stack, but replaces every
    encoder block with a parallel MHSA + cgMLP Branchformer layer.
    """
    ...
    def __init__(
        ...
        if resolved_cgmlp_conv_kernel <= 0 or resolved_cgmlp_conv_kernel % 2 == 0:
            raise ValueError(
                "BranchformerEncoder requires an odd, positive cgMLP kernel size, "
                f"got {resolved_cgmlp_conv_kernel}"
            )
        if merge_conv_kernel <= 0 or merge_conv_kernel % 2 == 0:
            raise ValueError(
                "BranchformerEncoder requires an odd, positive merge_conv_kernel, "
                f"got {merge_conv_kernel}"
            )
        if cgmlp_expansion_factor <= 0:
            raise ValueError(f"cgmlp_expansion_factor must be positive, got {cgmlp_expansion_factor}")
        ...
        super().__init__(
            ...
            conv_kernel_size=resolved_cgmlp_conv_kernel,
            conv_norm_type='layer_norm',
        ...
        self.cgmlp_conv_kernel = resolved_cgmlp_conv_kernel
        self.cgmlp_expansion_factor = cgmlp_expansion_factor
        ...
        for layer_idx in range(n_layers):
            self.layers[layer_idx] = BranchformerLayer(
                d_model=d_model,
                d_ff=d_ff,
    ...
    def get_layer_type(self, layer_idx: int) -> str:
        if isinstance(self.layers[layer_idx], BranchformerLayer):
            return "branchformer"
        return "conformer"
