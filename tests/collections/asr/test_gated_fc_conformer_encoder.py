...
from nemo.collections.asr.modules.gated_fc_conformer_encoder import GatedFCConformerEncoder
from nemo.collections.asr.parts.submodules.gated_fc_modules import GatedFCBlock


class TestGatedFCConformerEncoder:
    @pytest.mark.unit
    def test_replaces_requested_layers(self):
        model = GatedFCConformerEncoder(
            feat_in=10,
            n_layers=8,
            d_model=8,
            feat_out=8,
            ...
            dropout_emb=0.0,
            gated_fc_layer_indices=[2, 3],
            gated_fc_expansion_factor=4,
        )

        assert isinstance(model.layers[2], GatedFCBlock)
        assert isinstance(model.layers[3], GatedFCBlock)
        assert model.get_layer_type(1) == "fastconformer"
        assert model.get_layer_type(2) == "gated_fc"
    ...
    def test_invalid_duplicate_indices(self):
        with pytest.raises(ValueError, match="duplicate"):
            GatedFCConformerEncoder(feat_in=10, n_layers=4, d_model=8, gated_fc_layer_indices=[1, 1])
